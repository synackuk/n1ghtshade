...
int restore_send_nor(restored_client_t restore, struct idevicerestore_client_t* client, plist_t build_identity, plist_t message)
{
	...
	while (iter) {
		...
		if(!strcmp(comp, "DeviceTree")) {
			if (build_identity_get_component_path(build_identity, comp, &comppath) < 0) {
				free(comp);
		...
		if((client->flags & FLAG_DOWNGRADE) && strcmp(component, "DeviceTree")) {
	...
	if(client->flags & FLAG_DOWNGRADE) {
		...
		unsigned int component_size = 0;
		const char* component = "iBoot";
		char* comppath = NULL;
		
		if (build_identity_get_component_path(build_identity, component, &comppath) < 0) {
			error("ERROR: Unable to get component path for %s\n", component);
			return -1;
		}
		...
		if (img3_retag_component(component, component_data, component_size, 'ibox', &nor_data, &nor_size) < 0) {
		...
		component_size = 0;
		if (flash_version_1) {
			plist_dict_set_item(norimage, "iBox", plist_new_data((char*)nor_data, (uint64_t)nor_size));
		}
		else {
			plist_array_append_item(norimage, plist_new_data((char*)nor_data, (uint64_t)nor_size));
		}
