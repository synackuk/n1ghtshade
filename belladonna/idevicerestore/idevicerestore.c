...
static void usage(int argc, char* argv[], int err)
{
	...
	"  -w, --downgrade	 Enables using the latest blobs\n" \
	"\n" \
...
int idevicerestore_start(struct idevicerestore_client_t* client)
{
	...
	if(client->flags & FLAG_DOWNGRADE) {
		...
		if (client->flags & FLAG_ERASE) {
			client->latest_build_identity = build_manifest_get_build_identity_for_model_with_variant(tss_manifest, client->device->hardware_model, RESTORE_VARIANT_ERASE_INSTALL);
			...
			client->latest_build_identity = build_manifest_get_build_identity_for_model_with_variant(tss_manifest, client->device->hardware_model, RESTORE_VARIANT_UPGRADE_INSTALL);
	...
	build_identity_print_information(client->latest_build_identity);
