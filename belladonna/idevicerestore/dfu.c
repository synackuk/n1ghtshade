...
int dfu_send_component(struct idevicerestore_client_t* client, plist_t build_identity, const char* component)
{
	...
	if (!client->image4supported && client->build_major > 8 && !(client->flags & FLAG_CUSTOM) && !(client->flags & FLAG_DOWNGRADE) && !strcmp(component, "iBEC")) {
	...
	if(client->ibss_load_hook && !strcmp(component, "iBSS")) {
		int ret = client->ibss_load_hook(client, data, size);
		if (ret != 0) {
			error("ERROR: Unable to send %s component\n", component);
			free(data);
			return -1;
		}
	}
...
int dfu_enter_recovery(struct idevicerestore_client_t* client, plist_t build_identity)
{
	...
	if(client->ibec_post_load) {
		return client->ibec_post_load(client);
	}
