...
int img3_retag_component(const char* component_name, const unsigned char* component_data, unsigned int component_size, uint32_t new_tag, unsigned char** img3_data, unsigned int *img3_size)
{
	img3_file *img3 = NULL;
	unsigned char* outbuf = NULL;
	unsigned int outsize = 0;

	if (!component_name || !component_data || component_size == 0 || !img3_data || !img3_size) {
		return -1;
	}

	info("Retagging IMG3 component %s...\n", component_name);
	...
	img3->header->image_type = new_tag;

	for(int i = 0; i < img3->num_elements; i += 1) {
		img3_element* element = img3->elements[i];
		if(element->type == kTypeElement) {
			uint32_t* tag_loc = (uint32_t*)(element->data + sizeof(img3_element_header));
			*(uint32_t*)tag_loc = new_tag;
		}
	}
