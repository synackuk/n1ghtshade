...
static struct option longopts[] = {
	...
	{ "latest-shsh", no_argument,    NULL, 'b' },
	{ "ibec-mode", no_argument,      NULL, 'r' },
	{ "latest-baseband", no_argument,      NULL, 'w' },
	{ NULL, 0, NULL, 0 }
};
...
static void usage(int argc, char* argv[], int err)
{
	...
	" -b, --latest-shsh      Restore with the SHSH blobs for latest iOS regardless of\n" \
	"                        version being restored\n" \
	" -r, --ibec-mode        Don't upload iBEC to device\n" \
	" -w, --latest-baseband  Use latest baseband instead of the one in the IPSW\n" \
...
int idevicerestore_start(struct idevicerestore_client_t* client)
{
	...
	if(client->flags & FLAG_LATEST_SHSH) {
		...
		if (client->flags & FLAG_ERASE) {
			tss_identity = build_manifest_get_build_identity_for_model_with_restore_behavior(tss_manifest, client->device->hardware_model, "Erase");
			if (tss_identity == NULL) {
			...
			tss_identity = build_manifest_get_build_identity_for_model_with_restore_behavior(tss_manifest, client->device->hardware_model, "Update");
			if (!tss_identity) {
				tss_identity = build_manifest_get_build_identity_for_model(tss_manifest, client->device->hardware_model);
			}
		}
		plist_free(tss_manifest);
	...
	if (tss_enabled) {
		...
		if (get_tss_response(client, tss_identity, &client->tss) < 0) {
	...
	if (client->mode->index == MODE_DFU) {
		...
		if(!(client->flags & FLAG_NO_IBEC_UPLOAD)) {
			mutex_lock(&client->device_event_mutex);
	...
	if (tss_identity && client->flags & FLAG_LATEST_SHSH)
		plist_free(tss_identity);
...
int main(int argc, char* argv[]) {
	...
	while ((opt = getopt_long(argc, argv, "dhrcewsxtbpli:u:nC:ky", longopts, &optindex)) > 0) {
		switch (opt) {
		case 'h':
			usage(argc, argv, 0);
			return 0;
